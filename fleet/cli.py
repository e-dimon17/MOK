"""Fleet console entrypoints (wired in pyproject):
...
from fleet.attestation.challenge import (
    DEFAULT_DEADLINE_S,
    DEFAULT_INNER_STEPS,
    AttestationChallenge,
    make_challenge,
)
from fleet.attestation.reference_step import AttestationResponse, run_reference
from fleet.attestation.verify import Verifier, judge
from fleet.calibration.adam_ab import DEFAULT_K, DEFAULT_THRESHOLD_NATS, run_adam_ab
from fleet.calibration.local_harness import local_manifest
from fleet.calibration.rehearsal import run_calibration_windows
...
from fleet.onboarding.preflight import run_preflight
from fleet.onboarding.wallet_setup import (
...
def _add_config_args(p: argparse.ArgumentParser, *, required: bool = True) -> None:
    p.add_argument("--config", required=required, help="base RunConfig YAML (subnet/configs/base.yaml)")
    p.add_argument(
...
def build_calibrate_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="mok-calibrate", description="Fleet calibration suite.")
    sub = p.add_subparsers(dest="cmd", required=True)
    ...
    s = sub.add_parser("sweep", help="MoKConfig grid sweep -> subnet/configs/mok_tuned.yaml")
    common(s)
...
def _load_dataset(data_dir: str | Path) -> tuple[DatasetShardIndex, Callable[[int], Path]]:
    """The dataprep shard tree: ``shard_index.json`` + content-addressed shard files."""
    root = Path(data_dir)
