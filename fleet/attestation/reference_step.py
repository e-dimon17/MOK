...
training path (fleet bring-up: "run a reference 20-step deterministic MoK
...
minimal ``RunManifest`` — so ``WindowBatchPlan`` and ``subnet.core.inner_loop.
...
``tests/unit/test_fleet_reference_step.py``):
...
from subnet.core.window_runner import RunnerComm, SingleNodeComm, TorchDistRunnerComm, build_window_plan
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="python -m fleet.attestation.reference_step",
