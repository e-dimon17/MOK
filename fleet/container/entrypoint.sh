#!/usr/bin/env bash
# ============================================================================
# Role dispatch for the blessed container.
...
case "${ROLE}" in
    miner)
        ...
        exec "${TORCHRUN[@]}" -m subnet.miner.main "$@"
        ;;
    validator)
        require_env R2_ACCOUNT_ID R2_BUCKET_NAME \
            R2_WRITE_ACCESS_KEY_ID R2_WRITE_SECRET_ACCESS_KEY \
    ...
    auditor)
        require_env R2_ACCOUNT_ID R2_BUCKET_NAME \
            BT_WALLET_NAME BT_WALLET_HOTKEY BT_NETWORK BT_NETUID
        exec python -m subnet.auditor.main "$@"
        ;;
    attest)
        # 8-rank reference run; rank 0 prints the AttestationResponse JSON.
        exec "${TORCHRUN[@]}" -m fleet.attestation.reference_step "$@"
        ;;
    calibrate)
        ...
        ;;
    healthcheck)
        exec python -m fleet.ops.healthcheck "$@"
        ;;
    *)
