"""Step G: vLLM plugin — pure name-mapping tables + import-guard behavior.
...
parameter names of a tiny F/hf_model checkpoint) and the lazy guard that
...
import G.vllm_plugin.mok_moe_vllm as plugin
...
def test_mapping_covers_every_real_checkpoint_tensor() -> None:
    """Every tensor a converted checkpoint actually ships (tiny F/hf_model
...
def test_registry_constants() -> None:
    assert HF_ARCHITECTURE == "MokMoeForCausalLM"  # == F/hf_model class name
...
def test_model_class_access_raises_clear_error_without_vllm() -> None:
    ...
    with pytest.raises(ImportError, match="vllm>=0.8"):
        from G.vllm_plugin.mok_moe_vllm import MokMoeForCausalLM_vLLM  # noqa: F401
...
def test_importing_plugin_never_imports_vllm() -> None:
    ...
    assert "G.vllm_plugin.mok_moe_vllm" in sys.modules
