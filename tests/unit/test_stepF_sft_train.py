"""Step F: sft.yaml parsing, LR schedule math, fsdp2 config sanity (no TRL import)."""
...
def test_load_settings_parses_shipped_yaml() -> None:
    settings = load_settings(REPO / "F" / "configs" / "sft.yaml")
    ...
    assert settings.fsdp_config == "F/configs/fsdp2.yaml"
...
def test_fsdp2_yaml_references_our_layer_class() -> None:
    with open(REPO / "F" / "configs" / "fsdp2.yaml", encoding="utf-8") as f:
