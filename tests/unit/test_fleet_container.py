"""Inspection tests for fleet/container/* — the blessed-image contract lines.
...
CONTAINER_DIR = Path(__file__).resolve().parents[2] / "fleet" / "container"
...
def test_entrypoint_role_dispatch() -> None:
    ...
    assert "-m subnet.miner.main" in ENTRYPOINT
    assert "python -m subnet.validator.main" in ENTRYPOINT
    assert "python -m subnet.auditor.main" in ENTRYPOINT
    assert "-m fleet.attestation.reference_step" in ENTRYPOINT
    assert "mok-calibrate" in ENTRYPOINT
    assert "python -m fleet.ops.healthcheck" in ENTRYPOINT
