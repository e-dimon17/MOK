...
```
Notes:
- Golden vectors pinned in tests/unit/test_stepA_manifest.py (shard leaf aeb002c6... and merkle root d1d9efe6... on a fixed uint16 arange fixture) and tests/unit/test_stepA_shard_writer.py (65536*4096*2 == 536870912 == 512 MiB), each marked '# consensus constant — change requires SPEC_VERSION bump'.
...
```
Notes:
- lr_at warmup convention: step s in [0, warmup) gets peak*(s+1)/warmup_steps — peak is reached at s = warmup_steps-1 and there is no dead zero-LR step 0. wsd_linear_decay measures decay progress in tokens: (s - warmup_steps)*tokens_per_inner_step against decay_total_tokens, clamped at 0.0. D-phase LRSpec overlays must therefore set warmup_steps to the global step where decay begins. kind='const' ignores warmup entirely. Golden values pinned as consensus constants in test_phase.py.
...
C/core/pseudo_grad.py (the upstream implementation distributed.py:364-545 adapted, non-DTensor, credited in NOTICE):
...
```
C/core/exchange.py (all async unless noted):
...
H/verify_bundle.py:
  ...
  build_parser(); main(argv=None) -> int  # 0 iff ok; --json full report; python -m H.verify_bundle <dir>
...
H/replay_window.py:
  ...
  _replay(...)  # heavy path behind main(): load_run_config + config_hash must equal manifest.config_hash; init_model(cfg.model, seed=42, device, backend); theta_start via C.core.checkpoint.load_master_state(model, ckpt_dir) if present else direct DCP FileSystemReader on <dir>/model per checkpoint layout contract; C.core.replay.WindowReplayer(manifest=, cfg=, model=).replay(uid=, window=)

H/run_evals.py (console script mok-eval):
...
All key names re-exported from H/__init__.py; `import H` loads no heavy deps (asserted in a smoke run).
```
...
```
## stepB (drop-in section for docs/INTERNAL_API.md)

B/attestation/challenge.py:
  ATTEST_DOMAIN=b'attest.v1'; DEFAULT_DEADLINE_S=420.0; DEFAULT_INNER_STEPS=20; BASE_CONFIG_PATH/TOY4L_CONFIG_PATH (resolved via C package)
...
B/attestation/reference_step.py:
...
B/calibration/local_harness.py (production loopback rig, promoted from the test_window_runner fixture pattern):
...
B/calibration/sweep.py:
  SweepPoint(FrozenModel: fwd_num_comm_sms, bwd_num_comm_sms, minibatch_size); SweepResult(point, mok: MoKRuntimeConfig, mean_window_s, final_loss); DEFAULT_TUNED_PATH=C/configs/mok_tuned.yaml
