"""Attestation challenges — block-hash-seeded toy training runs (playbook step B).
...
the 4-layer toy model from ``C/configs/toy4L.yaml`` — from a chain-published
...
(``B.attestation.verify``).

Consensus derivation (SPEC_VERSION-bound, golden-pinned in
``tests/unit/test_stepB_challenge.py``), in the style of
...
#: The reference run is a 20-inner-step toy window (playbook step B).
DEFAULT_INNER_STEPS = 20

_MASK63 = (1 << 63) - 1
_HEX16 = re.compile(r"^[0-9a-f]{16}$")

_C_CONFIGS = Path(C.__file__).resolve().parent / "configs"
BASE_CONFIG_PATH = _C_CONFIGS / "base.yaml"
TOY4L_CONFIG_PATH = _C_CONFIGS / "toy4L.yaml"


def toy4l_overlay() -> dict[str, Any]:
    """The ``C/configs/toy4L.yaml`` overlay dict, verbatim (the challenge model shape)."""
...
class AttestationChallenge(FrozenModel):
    ...
    model_overlay: dict[str, Any]  # config overlay over C/configs/base.yaml (toy4L shape)
...
def derive_expected(
    ...
    *,
    ...
    (``B.attestation.reference_step.run_reference``), so expected and actual
