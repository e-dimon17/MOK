...
Decision rule (playbook step B): if the final-loss penalty of resetting every
...
mirror of ``C.core.inner_loop.InnerLoop.run_window`` with exactly ONE degree
...
``tests/unit/test_stepB_adam_ab.py``, so the A/B measures optimizer dynamics,
...
from C.core.inner_loop import IGNORE_INDEX, InnerLoop
from C.core.moe_health import MoeHealth
from C.core.phase import accum_at, lr_at, resolve_phase
from C.core.window_runner import build_window_plan, run_state_at
from C.core.zero1 import Comm, SingleProcessComm, Zero1Adam, flat_grad_all_reduce
