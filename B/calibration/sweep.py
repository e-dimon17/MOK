"""MoKConfig sweep — tune (comm SMs, minibatch) on the real fleet (step B).
...
the timed region) and emits the winner as ``C/configs/mok_tuned.yaml`` — an
...
DEFAULT_TUNED_PATH = Path(C.__file__).resolve().parent / "configs" / "mok_tuned.yaml"
...
def emit_tuned_yaml(
    ...
    header = (
        "# mok_tuned.yaml — MoK runtime knobs pinned by the step-B calibration sweep.\n"
