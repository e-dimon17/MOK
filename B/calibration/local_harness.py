...
promoted into production code (task spec): a real ``C.core.window_runner.
WindowRunner`` in ``self_leader`` bootstrap mode, wired to

  - ``MemoryStorage`` — a filesystem-backed stand-in implementing the exact
    ``mok_core.storage.StorageClient`` surface that ``C.core.exchange`` and
    ``C.core.checkpoint`` consume (put/get/list/head/gather with the same
...
Calibration (``B.calibration.rehearsal`` / step-B dress rehearsal) and tests
