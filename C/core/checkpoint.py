...
CHECKPOINT LAYOUT CONTRACT (consumed by step F's converter and by every node):
...
def _quiet_single_process(no_dist: bool):
    ...
    path that IS the intent (tests, step F, rank-0 saves), not a problem."""
...
class Checkpointer:
    ...
    single-process path (tests, step F); a torchrun rank group passes False.
    """
...
async def catch_up(
    ...
    *,
    ...
    ``exchange`` is the protocol I/O namespace (pass ``C.core.exchange`` or a
