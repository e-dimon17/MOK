"""`mok-data` — the step A data-preparation application.
...
from A.pipeline.build_manifest import (
    MANIFEST_FILENAME,
    TOKENIZER_FILENAME,
...
from A.pipeline.upload import upload_dataset_sync
from A.pipeline.verify import verify_local
...
def _cmd_dedup(args: argparse.Namespace) -> int:
    cfg = load_corpus_config(args.corpus_config)
    if args.parallel:
        from A.pipeline.dedup_parallel import dedup_parallel  # noqa: PLC0415
...
def _cmd_tokenize(args: argparse.Namespace) -> int:
    ...
    if args.workers:
        from A.pipeline.tokenize_pack import tokenize_parallel  # noqa: PLC0415
...
def _cmd_shard(args: argparse.Namespace) -> int:
    cfg = load_corpus_config(args.corpus_config)
    from A.pipeline.tokenize_pack import discover_token_files  # noqa: PLC0415
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="mok-data", description="MoK subnet step A: data preparation")
    p.add_argument(
        "--smoke",
        nargs="+",
        metavar="TEXT_FILE",
