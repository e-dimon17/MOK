...
from subnet.core.certificate import WindowCertificate, build_certificate
from subnet.core.checkpoint import Checkpointer, CheckpointMeta
from subnet.core.exchange import get_certificate, put_aggregator_object, put_certificate, put_debug_slices
from subnet.core.rollback import RollbackDecision, RollbackState, RollbackStateMachine, SpikeDetector
from subnet.core.window_runner import RunState
from subnet.miner.bootstrap import OWNER_UID, NodeContext, resolve_leader_uid
...
class LeaderDuties:
    ...
    def observe_probe_loss(
        ...
        if self.rollback.target_window is not None:
            ...
            from subnet.core.rollback import RollbackVote  # noqa: PLC0415 — local to the voting path
