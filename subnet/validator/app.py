...
gate has closed (i.e. the validator trails the chain head by one window):
  ...
  (b) leader only: publish the window certificate + aggregator mirror FIRST so
      miners can gather (`subnet.validator.leader`);
  (c) evaluate payloads at θ_start(w) — loss deltas on deterministic pools →
      gradient scores, BinaryEMA, OpenSkill (`subnet.validator.evaluator`);
...
from subnet.core.checkpoint import CatchUpError, CertificatePendingError, Checkpointer
from subnet.core.compress import unpack_12bit_indices
from subnet.core.exchange import ExchangeError, gate_check, get_debug_slices
from subnet.core.overlap import determine_offender, index_overlap_report, severity
from subnet.core.payload import PayloadError, WindowPayload, deserialize, validate_structure
from subnet.core.phase import resolve_phase
from subnet.core.rollback import RollbackState, RollbackStateMachine, SpikeDetector
from subnet.core.scoring import BinaryEMA, OpenSkillBook, final_score, sync_score
from subnet.core.slashing import SlashLedger
from subnet.core.window_runner import DENSE_SUFFIX, run_state_at
