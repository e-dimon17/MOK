...
class AuditReportLike(Protocol):
    """Minimal shape of an audit verdict (subnet/core/replay.py's AuditReport satisfies it)."""
...
class SlashLedger:
    ...
    def overlap(self, uid: int, window: int, multiplier: float, naughty: bool) -> SlashRecord:
        """Sanction from subnet/core/overlap.severity for the offender of a flagged pair."""
        st = self._uid(uid)
        st.apply_multiplier(window, multiplier)
