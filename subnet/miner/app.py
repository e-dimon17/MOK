...
Wraps `subnet.core.window_runner.WindowRunner` (the exact wiring proven by the
...
from subnet.core.checkpoint import CatchUpError, CertificatePendingError, Checkpointer, CheckpointMeta
from subnet.core.compress import ErrorFeedback
from subnet.core.exchange import ExchangeError, put_debug_slices, put_telemetry
...
class MinerApp:
    ...
    certify themselves exactly like the fleet calibration bootstrap.
    """
    ...
    async def _prefetch_next(self, window: int) -> None:
        ...
        try:
            phase = resolve_phase(ctx.manifest, ctx.cfg, window)
            from subnet.core.window_runner import build_window_plan  # noqa: PLC0415 — cycle-free but heavy
