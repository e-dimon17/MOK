...
This module owns everything the three apps have in common:
  ...
  * The local-harness stack (`ScriptedChain`, `MemoryStorage`, `LoopbackClock`,
    `_FallbackHarness`) used by `--local-harness` runs (fleet calibration, CPU
    tests) when `fleet.calibration.local_harness` has not been installed yet.
  ...
  * `catch_up_replica` — the thin app-side wrapper over `subnet.core.checkpoint.catch_up`.
...
from subnet.core.checkpoint import CatchUpReport, Checkpointer, catch_up
...
class LocalHarness(Protocol):
    """What `--local-harness` must supply. `fleet.calibration.local_harness.
...
def dataset_shard_key(name: str, leaf_hex: str) -> str:
    ...
    (`datasets/<name>/shard-<hash16>.bin`, mirroring dataprep's local filenames).
...
class _FallbackHarness:
    ...
    chain, memory object store, loopback clock. Used when fleet bring-up's
...
def _load_local_harness(cfg: RunConfig, *, root: Path, uid: int) -> LocalHarness:
    """Prefer the fleet harness when it exposes the bootstrap contract.

    fleet's `LocalLoopbackHarness` may exist with a different (runner-rig) shape;
    ...
    """
    try:
        from fleet.calibration.local_harness import LocalLoopbackHarness  # noqa: PLC0415
    except ImportError:
        log.info("fleet.calibration.local_harness not installed — using in-memory fallback harness")
...
async def materialize_replica(
    ...
    init via `fleet.onboarding.fetch_and_verify_init` (lazy import; bitwise-checked
    ...
    the returned state is θ_start(from_window + 1); -1 means θ_init. fleet's init
    ...
    try:
        from fleet.onboarding import fetch_and_verify_init  # noqa: PLC0415 — optional fleet hook
    except ImportError:
...
def build_arg_parser(role: str) -> argparse.ArgumentParser:
    ...
    p.add_argument("--config", default="subnet/configs/base.yaml", help="base RunConfig YAML")
    p.add_argument(
...
async def bootstrap(
    ...
    *,
    ...
    `--local-harness` loads `fleet.calibration.local_harness` or the in-memory
...
#: dataprep dataset trees when they live apart from the owner's operational bucket.
...
def _bucket_from_env(hotkey_ss58: str) -> BucketCreds:
    """This node's own R2 bucket (WRITE pair) from the fleet onboarding env.
    Wire v2: the bucket is named after the hotkey; R2_BUCKET_NAME, if set, must match."""
    from fleet.onboarding.wallet_setup import OnboardingError, write_creds_from_env  # noqa: PLC0415
