...
certified outer step — via C.core.replay.WindowReplayer and writes the
AuditReport JSON. Exit code 0 iff the replayed θ_end root matches the
committed one.

Usage::

    python -m H.replay_window --bundle ./bundle --window 1234 --miner-uid 17 \
...
All heavy imports (torch DCP, mok_core.model, C.core.replay) happen inside the
replay path, so `--help`/argument validation stay instant and unit-testable on
any machine. Verification of the surrounding bundle is H/verify_bundle.py's
...
from H.provenance import MANIFEST_FILENAME, audit_report_problems
...
def _load_theta_start(model: Any, ckpt_dir: Path) -> None:
    ...
    Prefers C/core/checkpoint.py's loader when it exposes one; otherwise reads
    ...
    """
    try:
        from C.core import checkpoint as ckpt_mod  # noqa: PLC0415
    except ImportError:
        ckpt_mod = None
...
def _replay(
    ...
    *,
    ...
    from C.core.replay import WindowReplayer  # noqa: PLC0415
