...
Method: 2 epochs over Tulu-3 + OpenHermes-2.5 + reasoning
traces, LR 5e-6 -> 1e-5 warmup then cosine to 0, seq 16384 with packing,
bf16, FSDP2 via `accelerate launch --config_file sft/configs/fsdp2.yaml`.
...
Launch:
    accelerate launch --config_file sft/configs/fsdp2.yaml -m sft.sft_train \
        --config sft/configs/sft.yaml
"""
...
from sft import data_prep

# --------------------------------------------------------------------------- #
# Settings (pure YAML -> dataclass; sft/configs/sft.yaml)
# --------------------------------------------------------------------------- #
...
class SFTSettings:
    ...
    ngram_n: int = 13
    fsdp_config: str = "sft/configs/fsdp2.yaml"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="mok-sft", description="SFT MoK-54B with TRL.")
    parser.add_argument("--config", type=Path, default=Path("sft/configs/sft.yaml"))
    return parser
