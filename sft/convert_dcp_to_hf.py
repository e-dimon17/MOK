...
Input (produced by subnet/core/checkpoint.py — the contract both sides follow):
...
# Consensus token strings — mirror dataprep/pipeline/tokenizer_train.py (sft must not import dataprep).
...
def infer_hf_config(sd: Mapping[str, torch.Tensor], *, head_dim: int | None = None) -> MokMoeConfig:
    ...
    run whose manifest deviates from those defaults (e.g. the context extension's rope_theta
    5e5 / 16k seq), pass `model_config=` to `convert` instead.
    """
    from mok_core.config import ModelConfig  # noqa: PLC0415
    from sft.hf_model.configuration_mok_moe import MokMoeConfig  # noqa: PLC0415 — transformers
...
def write_tokenizer_files(tokenizer_path: Path, out_dir: Path, *, model_max_length: int) -> None:
    """Copy the dataprep tokenizer.json and write HF tokenizer configs + template."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--tokenizer", type=Path, default=None, help="dataprep tokenizer.json to bundle")
    parser.add_argument("--dtype", choices=sorted(TORCH_DTYPES), default="bfloat16")
    parser.add_argument(
        "--model-config",
        type=Path,
        default=None,
        help="YAML with a `model:` section (e.g. subnet/configs/toy4L.yaml) pinning non-inferable fields",
    )
    parser.add_argument(
        "--no-state-root-check",
        action="store_true",
