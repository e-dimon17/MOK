"""TRL GRPOTrainer harness for RLVR, phase 2 of step G (`mok-rl`).
...
external vLLM servers (TRL server mode; endpoint list in G/configs/grpo.yaml,
one `trl vllm-serve`-compatible server per rollout node, each having
registered the MoK architecture via `G.vllm_plugin.register_mok_moe`).
...
from G.data.rlvr_code import build_code_prompts
from G.data.rlvr_math import build_math_prompts
from G.rewards.code_reward import SandboxMode, verify_code
from G.rewards.math_reward import verify_math
from mok_core.config.loader import load_yaml

# --------------------------------------------------------------------------- #
# Settings (pure YAML -> dataclass; G/configs/grpo.yaml)
# --------------------------------------------------------------------------- #
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="mok-rl", description="RLVR (GRPO) tune MoK-54B-chat with TRL + vLLM rollouts (step G)."
    )
    parser.add_argument("--config", type=Path, default=Path("G/configs/grpo.yaml"))
    return parser
