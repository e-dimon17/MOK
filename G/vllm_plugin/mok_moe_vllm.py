"""vLLM out-of-tree registration of the MoK-MoE architecture (step G).

`register_mok_moe()` registers `MokMoeForCausalLM` (the `architectures` entry
written by F/convert_dcp_to_hf.py) with vLLM's ModelRegistry, pointing at
`MokMoeForCausalLM_vLLM` below — a vLLM-native implementation of
F/hf_model/modeling_mok_moe.py built from vLLM primitives:
...
tests/unit/test_stepG_vllm_plugin.py.
"""
...
#: checkpoint's config.json (the F/hf_model class name).
...
VLLM_CLASS_PATH = "G.vllm_plugin.mok_moe_vllm:MokMoeForCausalLM_vLLM"
